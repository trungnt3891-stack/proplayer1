...
function getManifest() {
    return JSON.stringify({
        ...
        "description": "Bản Webview: Cực Nhanh, Không Treo Load, Auto-Click bỏ qua Popup, Giao diện vuốt Tiktok",
        "version": "1.3.6",
        "info": "Nguồn phim ngắn siêu hay. Tối ưu cho iOS với thuật toán cắt chuỗi siêu tốc và bot tự bấm đồng ý quảng cáo.",
...
// -----------------------------------------------------------------------------
// GIỮ NGUYÊN 100% CÁC HÀM TẢI URL VÀ MENU CỦA BẠN ĐỂ ĐẢM BẢO LOAD ĐƯỢC
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// ĐÃ SỬA: DÙNG STRING NATIVE SIÊU TỐC TRÁNH CRASH IOS VÀ LOAD NGAY LẬP TỨC
// -----------------------------------------------------------------------------
function parseListResponse(html, $url) {
    try {
        var items = [];
        
        // Cắt dữ liệu bằng .split thay vì dùng DOM _$() hay Regex gây quá tải
        var blocks = html.split('data-watch-url="');
        
        for (var i = 1; i < blocks.length; i++) {
            var block = blocks[i];
            
            // 1. Lấy Link
            var hrefEnd = block.indexOf('"');
            if (hrefEnd === -1) continue;
            var href = block.substring(0, hrefEnd);
            
            // 2. Lấy Tên Phim
            var title = "Đang cập nhật";
            var titleStart = block.indexOf('data-movie-title="');
            if (titleStart !== -1) {
                titleStart += 18;
                var titleEnd = block.indexOf('"', titleStart);
                if (titleEnd !== -1) title = block.substring(titleStart, titleEnd);
            }
            
            // 3. Lấy Ảnh
            var src = "";
            var imgStart = block.indexOf('src="');
            if (imgStart !== -1) {
                imgStart += 5;
                var imgEnd = block.indexOf('"', imgStart);
                if (imgEnd !== -1) src = block.substring(imgStart, imgEnd);
            }
            
            // 4. Lấy Số Tập
            var episode_current = "";
            var epStart = block.indexOf('episode-badge"');
            if (epStart !== -1) {
                var gt = block.indexOf('>', epStart);
                var lt = block.indexOf('<', gt);
                if (gt !== -1 && lt !== -1) {
                    episode_current = block.substring(gt + 1, lt).trim();
                }
            }
            
            // Xử lý chuẩn hoá Link & Ảnh tốc độ cao
            if (href.indexOf("http") == -1) href = BASEURL + href;
            var baseHref = href.split('?')[0]; 
            href = baseHref + "/1?lang=vi-VN";
            
            if (src.indexOf("http") == -1) src = BASEURL + src;
            var cleanThumb = src.replace(/&amp;/g, '&');
            
            if (href.indexOf("http") > -1 && href.indexOf("watch/") > -1) {
                items.push({
                    "id": href,
                    "title": title.trim(),
        ...
        return JSON.stringify({
            "items": [{
                "id": $url || "error_url",
                "title": "Lỗi Load: Vui lòng tải lại",
                "posterUrl": "",
                "backdropUrl": ""
            }],
...
// -----------------------------------------------------------------------------
// ĐÃ SỬA: RÚT GỌN TỐI ĐA THUẬT TOÁN ĐỂ HIỆN LINK XEM PHIM NGAY LẬP TỨC
// -----------------------------------------------------------------------------
function parseMovieDetail(html, url) {
    try {
        ...
        // Trích xuất thần tốc bằng indexOf không cần duyệt DOM
        var tStart = html.indexOf('<meta property="og:title" content="');
        if (tStart !== -1) {
            tStart += 35;
            var tEnd = html.indexOf('"', tStart);
            if (tEnd !== -1) lname = html.substring(tStart, tEnd);
        }

        var iStart = html.indexOf('<meta property="og:image" content="');
        if (iStart !== -1) {
            iStart += 35;
            var iEnd = html.indexOf('"', iStart);
            if (iEnd !== -1) limg = html.substring(iStart, iEnd);
        }

        var dStart = html.indexOf('<meta property="og:description" content="');
        if (dStart !== -1) {
            dStart += 41;
            var dEnd = html.indexOf('"', dStart);
            if (dEnd !== -1) ldes = html.substring(dStart, dEnd);
        }

        var cats = [];
        var catBlocks = html.split('movie-tag-pill"');
        for (var i = 1; i < catBlocks.length; i++) {
            var gt = catBlocks[i].indexOf('>');
            var lt = catBlocks[i].indexOf('<', gt);
            if (gt !== -1 && lt !== -1) {
                var txt = catBlocks[i].substring(gt + 1, lt).trim();
                if (txt) cats.push(txt);
            }
        }
        category = cats.join(" - ");

        var subStart = html.indexOf('movie-sub"');
        if (subStart !== -1) {
            var sgt = html.indexOf('>', subStart);
            var slt = html.indexOf('<', sgt);
            if (sgt !== -1 && slt !== -1) {
                episode_current = html.substring(sgt + 1, slt).trim();
            }
        }
...
// -----------------------------------------------------------------------------
// ĐÃ SỬA: SỬ DỤNG BOT AUTO-CLICK "BẤM HỘ" ĐỂ VƯỢT POPUP MÀ KHÔNG GÂY TREO WEB
// -----------------------------------------------------------------------------
...
// BỘ DOM ẢO _$ THEO CHUẨN CỦA VAX APP ĐÃ ĐƯỢC GIỮ LẠI LÀM FALLBACK
