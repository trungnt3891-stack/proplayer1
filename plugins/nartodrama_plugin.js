...
function getManifest() {
    return JSON.stringify({
        ...
        "description": "Bản Webview: Fix lỗi Popup, Tự động click Đồng ý, Ẩn banner",
        "version": "1.3.1",
...
// -----------------------------------------------------------------------------
// CHỈ TRẢ VỀ DUY NHẤT 1 LINK VÀO WEBVIEW
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// ĐÃ FIX: TIÊM LỆNH XÓA/ẨN VÀ TỰ AUTO-CLICK CHUẨN XÁC NÚT SWEETALERT (SWAL)
// -----------------------------------------------------------------------------
    ...
    try {
        var killAdsCssJs = `
            (function() {
                ...
                // Ẩn Header, Menu cạnh, Footer, Gợi ý, SEO block, Quảng cáo và Banner tải app
                // Cố tình làm tàng hình .swal2-container để tránh nháy giật màn hình khi script auto-click
                style.innerHTML = 'header, .topbar, .topbar-inner, footer, .site-footer-wrap, .site-footer, .desktop-sidebar-left, .desktop-sidebar-right, .player-seo-block, .player-random-section, .watch-history-fab, .nd-auth-modal, .share-buttons, .adsense-wrap, .adsense-box, [class*="ad-"], [id*="ad-"], iframe[src*="ads"], .player-subscribe-overlay { display: none !important; opacity: 0 !important; pointer-events: none !important; z-index: -9999 !important; } .swal2-container { opacity: 0 !important; pointer-events: none !important; z-index: -9999 !important; } body, html { margin: 0 !important; padding: 0 !important; background: #000 !important; overflow: hidden !important; height: 100vh !important; } .player-page, .player-shell, .shorts-wrap { margin: 0 !important; padding: 0 !important; max-width: 100% !important; width: 100% !important; height: 100dvh !important; } .shorts-stage { width: 100vw !important; height: 100dvh !important; border-radius: 0 !important; max-width: 100% !important; }';
                document.head.appendChild(style);

                setInterval(function() {
                    // Tự động nhấn nút Confirm (Đồng ý và tiếp tục xem) của thư viện SweetAlert2
                    var swalConfirm = document.querySelector('.swal2-confirm');
                    if (swalConfirm) {
                        try { swalConfirm.click(); } catch(e){}
                    }

                    // Fallback Click nếu có các popup Đồng ý dạng button thông thường khác
                    ...
                    // Xóa triệt để các background phủ đen mờ màn hình
                    var popups = document.querySelectorAll('.last-watch-popup, .watch-history-popup, .nd-auth-backdrop, .modal-backdrop, .popup-overlay');
                    for (var j = 0; j < popups.length; j++) {
                        if (popups[j]) popups[j].remove();
                    }

                    // Giải phóng tính năng vuốt cuộn màn hình bị đóng băng bởi popup
