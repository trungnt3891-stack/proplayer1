...
function getManifest() {
    return JSON.stringify({
        ...
        "description": "Bản Webview: Vuốt dọc tự chuyển tập, Chặn Quảng Cáo, Mở Khóa Cuộn Trang",
        "version": "1.2.3",
...
function parseListResponse(html, $url) {
    try {
        ...
        // Phương pháp 1: Chặt chuỗi theo thẻ <article (Chuẩn xác nhất cho giao diện Narto Drama hiện tại)
        var blocks = html.split('<article');
        if (blocks.length > 1) {
            for (var i = 1; i < blocks.length; i++) {
                var block = blocks[i];
                var urlMatch = block.match(/data-watch-url=["']([^"']+)["']/i) || block.match(/href=["']([^"']+)["']/i);
                var titleMatch = block.match(/data-movie-title=["']([^"']+)["']/i) || block.match(/<img[^>]*alt=["']([^"']+)["']/i);
                var imgMatch = block.match(/<img[^>]*src=["']([^"']+)["']/i) || block.match(/data-src=["']([^"']+)["']/i) || block.match(/srcset=["']([^"'\s]+)/i);
                var epMatch = block.match(/episode-badge[^>]*>([^<]+)/i);

                if (urlMatch && imgMatch) {
                    var href = urlMatch[1].trim();
                    var src = imgMatch[1].trim();
                    var title = titleMatch ? titleMatch[1].trim() : "Phim Mới";
                    var ep = epMatch ? epMatch[1].trim() : "HD";

                    if (href.indexOf("http") === -1) href = BASEURL + (href.startsWith('/') ? '' : '/') + href;
                    
                    // Giải mã cấu trúc NextJS Image
        ...
        // Phương pháp 2: Fallback bằng DOM Ảo
        if (items.length === 0) {
            _$(html).find(".card, .movie-card, a[href*='/detail/']").each(function() {
                ...
                if (src.indexOf("/_next/image?url=") !== -1) {
                    var sm = src.match(/url=([^&]+)/);
                    if (sm) src = decodeURIComponent(sm[1]);
                }
...
// -----------------------------------------------------------------------------
// CHỈ TRẢ VỀ DUY NHẤT 1 LINK VÀO WEBVIEW ĐỂ TỰ CHỌN TẬP BÊN TRONG
// -----------------------------------------------------------------------------
function parseMovieDetail(html, url) {
    try {
        ...
        // Fix Lỗi Regex Lookbehind (?<=) -> Dùng Match thuần túy cho iOS
        if (url.indexOf("/detail/watch/") > -1) {
            var matchWatch = url.match(/\/watch\/([^/?]+)/i);
            if (matchWatch && matchWatch[1]) {
                watchLink = BASEURL + "/watch/" + matchWatch[1].replace(/\/\d+$/, "") + "/1?lang=vi-VN";
            }
        } else if (url.indexOf("/detail/") > -1) {
            var matchDetail = url.match(/\/detail\/([^/?]+)/i);
            if (matchDetail && matchDetail[1]) {
                watchLink = BASEURL + "/watch/" + matchDetail[1].replace(/\/\d+$/, "") + "/1?lang=vi-VN";
            }
        }
        ...
        servers.push({
            ...
            episodes: [{
                id: watchLink, // TRUYỀN CHÍNH XÁC URL ĐỂ VÀO WEBVIEW
...
// -----------------------------------------------------------------------------
// ĐÃ SỬA: XÓA SỔ HOÀN TOÀN CÁC LỚP PHỦ VÀ MỞ KHÓA CUỘN TRANG (UNLOCK SCROLL)
// -----------------------------------------------------------------------------
    ...
    try {
        var killAdsCssJs = `
            (function() {
                ...
                // Đồng thời ép overflow-y: auto !important để lấy lại khả năng cuộn trang.
                style.innerHTML = '.topbar, .topbar-inner, .site-footer-wrap, .site-footer, .share-buttons, .adsense-wrap, .adsense-box, [class*="ad-"], [id*="ad-"], .watch-history-fab, iframe[src*="ads"], .nd-auth-modal, .global-search, .hamburger { display: none !important; opacity: 0 !important; pointer-events: none !important; z-index: -9999 !important; } body, html { margin: 0 !important; padding: 0 !important; background: #000 !important; overflow-x: hidden !important; overflow-y: auto !important; }';
                document.head.appendChild(style);

                setInterval(function() {
                    // TIÊU DIỆT TẬN GỐC (Remove) các lớp phủ rác làm chặn màn hình
                    var popups = document.querySelectorAll('.swal2-container, .last-watch-popup, .watch-history-popup, .nd-auth-modal, .nd-auth-backdrop, .sweet-alert, .modal-backdrop, .popup-overlay');
                    for (var j = 0; j < popups.length; j++) {
                        if (popups[j]) popups[j].remove(); // Bắt buộc dùng remove() thay vì display: none
                    }

                    // Mở khóa các class chống cuộn chuột của SweetAlert gây ra
                    document.body.classList.remove('swal2-shown', 'swal2-height-auto', 'modal-open', 'overflow-hidden');
                    document.documentElement.classList.remove('swal2-shown', 'swal2-height-auto', 'modal-open', 'overflow-hidden');
                    
                    // Khôi phục hành vi chạm của màn hình
                    document.body.style.pointerEvents = 'auto';

                    // Tự động bấm play hoặc đồng ý tiếp tục xem
                    var btns = document.querySelectorAll('button, a, .last-watch-btn, .swal2-confirm');
                    for (var k = 0; k < btns.length; k++) {
                        var t = btns[k].innerText || btns[k].textContent || '';
                        if (t.indexOf('Đồng ý') > -1 || t.indexOf('Continue') > -1 || t.indexOf('Tiếp tục') > -1 || t.indexOf('Agree') > -1 || t.indexOf('Bắt đầu') > -1) {
