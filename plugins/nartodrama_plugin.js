...
function getManifest() {
    return JSON.stringify({
        "id": "narto_drama",
        "name": "Narto Drama",
        "description": "Bản Fix Master: Chơi trực tiếp m3u8 (Native AVPlayer), Fix triệt để lỗi kẹt Tập 1, Vuốt dọc TikTok",
        "version": "3.1.6",
...
function parseDetailResponse(html, url) {
    try {
        ...
        // Lục trong JSON để tìm Link Play của tập hiện tại
        var scriptMatch = html.match(/const\s+episodeItemsUnlocked\s*=\s*(\[[\s\S]*?\]);/i);
        if (scriptMatch && scriptMatch[1]) {
            try {
                var episodes = JSON.parse(scriptMatch[1]);
        ...
        // Dùng Regex quét HTML nếu API bị sập/đổi format
        if (!streamUrl) {
        ...
        // DỰ PHÒNG KHI KHÔNG TÌM THẤY LINK: Ép Webview tự click Đồng ý và Xóa div Popup
        var autoClickJs = `
...
// =============================================================================
// BỘ DOM ẢO _$ (CHUẨN VAX) - Giữ nguyên không thay đổi
// =============================================================================
