...
function getManifest() {
    return JSON.stringify({
        ...
        "description": "Bản Webview: Fix lỗi Load, Auto-Click bỏ qua Popup, Bật Tìm kiếm",
        "version": "1.4.2",
...
// [ĐÃ SỬA] TỐI ƯU HÀM LẤY ĐƯỜNG DẪN TÌM KIẾM
    ...
    try {
        ...
        if (filtersJson) {
            try {
                ...
                try {
                    var filters2 = JSON.parse(fixedJson);
...
// -----------------------------------------------------------------------------
// [ĐÃ SỬA] PARSER: BẮT THÊM THUỘC TÍNH DATA-SRC ĐỂ TRÁNH MẤT BÌA PHIM
// -----------------------------------------------------------------------------
function parseListResponse(html, $url) {
    try {
        ...
        _$(html).find("article[class*='card']").each(function() {
            ...
            // FIX MẤT BÌA PHIM Ở TÌM KIẾM/TRANG CHỦ: Quét thêm data-src trước khi quét src
            var imgTag = this.find("img");
            var src = imgMatch = imgTag.attr("data-src") || imgTag.attr("src") || "";
...
// -----------------------------------------------------------------------------
// GIỮ NGUYÊN BẢN GỐC CỦA BẠN: CHỈ TRẢ VỀ 1 LINK WEBVIEW
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// BỘ DOM ẢO _$ THEO CHUẨN CỦA VAX APP (GIỮ NGUYÊN ĐỂ KHÔNG BỊ LỖI CHẠY)
// -----------------------------------------------------------------------------
