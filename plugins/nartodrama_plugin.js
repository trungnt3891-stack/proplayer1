...
function getManifest() {
    return JSON.stringify({
        "id": "narto_drama",
        "name": "Narto Drama",
        "description": "Bản Fix Master: Chơi trực tiếp m3u8, Bypass Native Popup chặn xem, Vuốt dọc TikTok",
        "version": "3.1.1", // Nâng version
        ...
        "type": "shortfilm",           
        "layoutType": "VERTICAL",      
        // CHUYỂN TỪ 'embedtoexoplay' SANG 'exoplayer' ĐỂ BỎ QUA HOÀN TOÀN WEBVIEW
...
function parseMovieDetail(html, url) {
    try {
        ...
        // Trích xuất luồng play_url trực tiếp từ Data JSON
        ...
        if (scriptMatch && scriptMatch[1]) {
            try {
                ...
                for (var i = 0; i < episodes.length; i++) {
                    ...
                    if (streamUrl) {
                        items.push({
                            id: streamUrl, // ID BÂY GIỜ CHÍNH LÀ RAW M3U8/MP4
...
// BỎ POPUP BẰNG CÁCH KHÔNG DÙNG WEBVIEW, TRẢ VỀ RAW URL
function parseDetailResponse(html, url) {
    try {
        var streamUrl = url;
        
        // Trong trường hợp ID được truyền vào không phải là m3u8, ta rà soát lại biến mảng
        ...
        // Bỏ Inject JS, trả thẳng URL Media File cho VAX ExoPlayer xử lý
        return JSON.stringify({
            "url": streamUrl,
            "isEmbed": false, // ÉP KHÔNG SỬ DỤNG WEBVIEW, ĐÁY THẲNG VÀO NATIVE PLAYER
