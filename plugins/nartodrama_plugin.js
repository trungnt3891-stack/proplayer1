...
function getManifest() {
    return JSON.stringify({
        ...
        "description": "Bản Webview Master: Vuốt dọc tự chuyển tập, Auto xoay dọc, Fix lỗi 404, Tự bấm Đồng Ý Ads",
        "version": "1.2.4",
        "info": "Nguồn phim ngắn siêu hay, vuốt như Tiktok. Tương thích hoàn toàn với iOS.",
...
function parseListResponse(html, $url) {
    try {
        ...
        _$(html).find("article[class*='card']").each(function() {
            ...
            // Loại bỏ thay thế chuỗi gây hỏng URL
            href = href.replace(/(^[\s\S]*?)\?[\s\S]*$/i,"$1?lang=vi-VN");
...
// -----------------------------------------------------------------------------
// ĐÃ SỬA: FIX LỖI "NOT FOUND" DO NỐI URL BỊ LỆCH CHUẨN CỦA NARTO DRAMA
// -----------------------------------------------------------------------------
function parseMovieDetail(html, url) {
    try {
        ...
        // TẠO LINK CHUẨN ĐỂ MỞ WEBVIEW KHÔNG BAO GIỜ BỊ 404 NOT FOUND
        var watchLink = url;
        var match = url.match(/\/detail\/(?:watch\/)?([^/?#]+)/i);
        if (match) {
            var slug = match[1];
            if (slug.toLowerCase() === "watch") {
                var m2 = url.match(/\/detail\/watch\/([^/?#]+)/i);
                if (m2) slug = m2[1];
            }
            watchLink = BASEURL + "/detail/watch/" + slug + "/1?lang=vi-VN";
        } else if (url.indexOf("/watch/") > -1) {
            var m3 = url.match(/\/watch\/([^/?#]+)/i);
            if (m3) {
                watchLink = BASEURL + "/detail/watch/" + m3[1] + "/1?lang=vi-VN";
            }
        }
        ...
        servers.push({
            ...
            episodes: [{
                id: watchLink, // TRUYỀN URL CHUẨN XÁC VÀO WEBVIEW
...
// -----------------------------------------------------------------------------
// ĐÃ SỬA: ÉP WEBVIEW VÀ TIÊM JAVASCRIPT/CSS ĐỂ XÓA MỌI RÁC, TỰ BẤM ĐỒNG Ý
// -----------------------------------------------------------------------------
        ...
        var killAdsCssJs = `
            (function() {
                ...
                setInterval(function() {
                    // Tự động bấm nút Đồng ý / Continue để video tự động phát
                    var btns = document.querySelectorAll('button, a, .swal2-confirm');
                    for (var k = 0; k < btns.length; k++) {
                        var t = btns[k].innerText || btns[k].textContent || '';
                        if (t.indexOf('Đồng ý') > -1 || t.indexOf('tiếp tục xem') > -1 || t.indexOf('Continue') > -1) {
                            try { btns[k].click(); } catch(e){}
                        }
                    }

                    // Tự động xóa các lớp phủ rác làm liệt cảm ứng
                    var popups = document.querySelectorAll('.swal2-container, .last-watch-popup, .watch-history-popup, .nd-auth-backdrop, .sweet-alert, .modal-backdrop, .popup-overlay');
                    for (var j = 0; j < popups.length; j++) {
                        // Nếu box đó KHÔNG chứa chữ đồng ý thì xóa luôn (để tránh xóa mất nút bấm quan trọng)
                        if (popups[j] && popups[j].innerHTML.indexOf('tiếp tục xem') === -1 && popups[j].innerHTML.indexOf('Đồng ý') === -1) {
                            popups[j].remove();
                        }
                    }

                    // Mở khóa thanh cuộn bị đóng băng bởi popup
