...
function getManifest() {
    return JSON.stringify({
        "id": "narto_drama",
        "name": "Narto Drama",
        "description": "Bản Fix Master: Link chuẩn từng tập, Tiêu diệt Popup, Kích hoạt lại Vuốt Dọc",
        "version": "3.2.1",
        ...
        "playerType": "embedtoexoplay" // QUAN TRỌNG: iOS IPA cần từ khóa này để dựng khung UI TikTok
    });
}
...
function parseMovieDetail(html, url) {
    try {
        ...
        return JSON.stringify({
            ...
            casts: "",
            director: "",
            // Ép thêm lớp bảo vệ dọc ở bên trong
            type: "shortfilm",
            layoutType: "VERTICAL"
        });
    } catch (e) {
...
function parseDetailResponse(html, url) {
    try {
        ...
        if (scriptMatch && scriptMatch[1]) {
            try {
                ...
                for (var i = 0; i < episodes.length; i++) {
                    ...
                    // Chỉ lấy m3u8 jika KHỚP ĐÚNG con số của tập người dùng vừa bấm
        ...
        // CÁCH 2: Tìm dalam mảng dari NextJS
        if (!streamUrl) {
            var nextDataMatch = html.match(/id="__NEXT_DATA__"[^>]*>(\{.*?\})<\/script>/i);
            if (nextDataMatch) {
                try {
        ...
        // Jika tidak ketemu URL, biarkan gagal diam-diam, JANGAN pake Webview
