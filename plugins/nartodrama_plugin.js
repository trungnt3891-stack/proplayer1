...
function getManifest() {
    return JSON.stringify({
        ...
        "description": "Bản Webview: Vuốt dọc tự chuyển tập, Chặn Quảng Cáo, Mở Menu Tập Phim Mượt",
        "version": "1.2.1",
        "info": "Nguồn phim ngắn siêu hay. Đã cấu hình tối ưu giao diện lướt dọc mượt mà, không dính quảng cáo.",
        "baseUrl": "https://edge.narto-drama.com",
        "iconUrl": "https://edge.narto-drama.com/narto-drama-logo-compressed.png",
        "isEnabled": true,
        // BẬT CẤU HÌNH AUTO XOAY DỌC MÀN HÌNH TIKTOK:
        "type": "shortfilm",         // [ÉP GIAO DIỆN PHIM NGẮN TIKTOK]
        "layoutType": "VERTICAL",    // [ÉP AUTO XOAY DỌC MÀN HÌNH]
        "playerType": "webview",     // [CHUYỂN SANG WEBVIEW ĐỂ TỰ VUỐT CỦA WEB]
        "subtitleCat": true
    })
};
...
// =============================================================================
// PARSERS MẠNH MẼ: DÙNG REGEX THAY VÌ DOM ẢO ĐỂ TRÁNH LỖI IOS
// =============================================================================
...
function parseListResponse(html, $url) {
    try {
        ...
        // SỬ DỤNG REGEX ĐỂ BẮT DATA ĐẢM BẢO 100% HIỆN ẢNH BÌA VÀ PHIM
        ...
        while ((match = regex.exec(html)) !== null) {
            ...
            // Làm sạch Title (Loại bỏ các chữ Lồng Tiếng, Dubbed)
        ...
        // Phương pháp Fallback bằng DOM Ảo (dành cho các layout phụ)
        if (items.length === 0) {
...
function parseMovieDetail(html, url) {
    try {
        ...
        if (rmatch && rmatch[1]) {
            lname = rmatch[1].split("-")[0].trim(); // Bỏ đuôi trang web
        ...
        // TRẢ VỀ NÚT "XEM NGAY" VÀ CHỈ ĐỊNH VÀO WEBVIEW
        var servers = [];
        servers.push({
            name: "Lướt Tự Động (Webview)",
            episodes: [{
                id: url, // TRUYỀN NGUYÊN LINK VÀO WEBVIEW KHÔNG CẮT GHÉP URL
...
// -----------------------------------------------------------------------------
// ĐÃ SỬA LỖI: SỬA LẠI ĐOẠN JS TIÊM VÀO ĐỂ KHÔNG CHẶN TÍNH NĂNG CHỌN TẬP
// -----------------------------------------------------------------------------
    ...
    try {
        ...
        var killAdsCssJs = `
            (function() {
                ...
                // CHÚ Ý: Bỏ ẩn class .modal và bỏ overflow:hidden để không block menu chọn tập phim
                style.innerHTML = '.topbar, .topbar-inner, .site-footer-wrap, .site-footer, .share-buttons, .adsense-wrap, .adsense-box, [class*="ad-"], [id*="ad-"], .watch-history-fab, iframe[src*="ads"], .nd-auth-modal, .global-search, .hamburger { display: none !important; opacity: 0 !important; pointer-events: none !important; z-index: -9999 !important; } body, html { background: #000 !important; }';
                document.head.appendChild(style);

                setInterval(function() {
                    // Xóa hẳn (remove) các lớp phủ rác và popup bắt đăng nhập để không đè lên màn hình
                    ...
                    for (var j = 0; j < popups.length; j++) {
                        if (popups[j]) popups[j].remove();
                    }

                    // Khôi phục khả năng cuộn bị khóa bởi SweetAlert
                    ...
                    for (var k = 0; k < btns.length; k++) {
                        ...
                        if (t.indexOf('Đồng ý') > -1 || t.indexOf('Continue') > -1 || t.indexOf('Tiếp tục') > -1 || t.indexOf('Bắt đầu') > -1) {
