...
function getManifest() {
    return JSON.stringify({
        "id": "narto_drama",
        "name": "Narto Drama",
        "description": "Bản Fix Master: Chơi trực tiếp m3u8, Fix kẹt tập, Auto xoay dọc màn hình",
        "version": "3.1.7",
        ...
        "type": "shortfilm",           
        "layoutType": "VERTICAL",      
        "playerType": "exoplayer" // QUAN TRỌNG: Phải là exoplayer để kích hoạt UI vuốt dọc TikTok
    });
}
...
// 1. CHỈ ĐỂ 5 MỤC NỔI BẬT Ở TRANG CHỦ ĐỂ TRÁNH LAG
...
// 2. DANH MỤC THỂ LOẠI VÀ MENU CHÍNH
...
// Bóc tách cẩn thận URL từng tập, đảm bảo không bị lỗi đường link
function parseMovieDetail(html, url) {
    try {
        ...
        // Bóc tách Base URL một cách cẩn thận để tránh lỗi với dấu '?'
        var cleanUrl = url.split('?')[0]; 
        var queryStr = url.indexOf('?') !== -1 ? url.substring(url.indexOf('?')) : "?lang=vi-VN";
        
        // Loại bỏ con số chỉ số tập ở đuôi nếu có
        var pathParts = cleanUrl.split('/');
        if (pathParts.length > 0 && !isNaN(pathParts[pathParts.length - 1]) && pathParts[pathParts.length - 1] !== "") {
            pathParts.pop(); 
        }
        var baseMoviePath = pathParts.join('/'); 

        var items = [];
        var seenEps = {};
        
        function addEpisode(epNum, overrideHref) {
            if (!seenEps[epNum]) {
                var epUrl = overrideHref ? overrideHref : (baseMoviePath + "/" + epNum + queryStr);
                if (epUrl.indexOf('http') !== 0) {
                    epUrl = BASEURL + (epUrl.startsWith('/') ? '' : '/') + epUrl;
                }
                items.push({
                    id: epUrl, // ID luôn là URL web chính xác để parseDetailResponse gọi
        ...
        // 1. Quét Data List Tập từ mảng Script
        ...
        if (scriptMatch && scriptMatch[1]) {
            try {
                ...
                for (var i = 0; i < episodes.length; i++) {
                    ...
                    addEpisode(epNum, null);
                }
            } catch (err) {}
        }

        // 2. Dự phòng: Bóc các Link tập trên giao diện
        if (items.length === 0) {
            ...
            for (var j = 0; j < epLinks.length; j++) {
                ...
                if (epHref) {
                    addEpisode(fallbackNum, epHref);
                }
            }
        }
        
        // 3. Dự phòng cấp 2 (Làm bảo hiểm)
        if (items.length === 0) {
            addEpisode(1, url);
        }

        // Đảm bảo các tập được sắp xếp theo đúng thứ tự 1, 2, 3...
        ...
        return JSON.stringify({
            ...
            servers: servers,
            lang: "Vietsub",
            layoutType: "VERTICAL", // Bọc thêm bảo hiểm ép chiều dọc cho VAX
            type: "shortfilm"
        });
    } catch (e) {
...
// Bắt chính xác m3u8 ứng với TỪNG TẬP được bấm vào
function parseDetailResponse(html, url) {
    try {
        var streamUrl = "";
        
        // 1. Lục trong JSON để tìm Link Play của tập hiện tại
        var scriptMatch = html.match(/const\s+episodeItemsUnlocked\s*=\s*(\[[\s\S]*?\]);/i);
        if (scriptMatch && scriptMatch[1]) {
            try {
                var episodes = JSON.parse(scriptMatch[1]);
                
                // Tách lấy con số đại diện cho tập trong URL (để so khớp)
                var cleanUrl = url.split('?')[0];
                var pathParts = cleanUrl.split('/');
                var currentEp = pathParts[pathParts.length - 1]; 
                
                for (var i = 0; i < episodes.length; i++) {
                    var ep = episodes[i];
                    var epNum = ep.number || ep.route_episode_number || (i + 1);
                    // Bắt trúng phóc cái URL m3u8 của tập đó
                ...
                // Backup nếu không match được số, cứ lấy đại phần tử đầu tiên
                if (!streamUrl && episodes.length > 0) {
                    streamUrl = episodes[0].play_url || episodes[0].direct_play_url;
                }
            } catch(e){}
        }

        // 2. Dùng Regex quét HTML nếu API bị sập/đổi format
        if (!streamUrl) {
        ...
        // Bỏ Webview, bắn trực tiếp vào Native Player của App
        if (streamUrl) {
            return JSON.stringify({
                "url": streamUrl,
                "isEmbed": false, 
        ...
        // Dự phòng Tối hậu: Ép Webview tự click Đồng ý và Xóa div Popup
        var autoClickJs = `
            setInterval(function() {
                ...
                for (var j = 0; j < divs.length; j++) {
                    var el = divs[j];
                    var htmlContent = (el.innerHTML || '').toLowerCase();
                    var zIdx = window.getComputedStyle(el).zIndex;
                    if (htmlContent.includes('quảng cáo trước khi xem') || (parseInt(zIdx) > 90 && htmlContent.includes('đồng ý'))) {
                        el.remove(); 
                    }
                }
            }, 800);
