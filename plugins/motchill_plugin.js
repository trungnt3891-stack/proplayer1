...
function getManifest() {
    return JSON.stringify({
        ...
        "description": "Bản Fix Chuẩn: Bắt đúng 100% ID, bỏ T Vietsub, sửa lỗi Unicode.",
        "version": "1.0.10",
        "baseUrl": BASEURL,
...
// =============================================================================
// PARSERS (GIỮ NGUYÊN CODE BẠN CUNG CẤP)
// =============================================================================
...
function transformMovieData(data) {
    ...
    data.servers.forEach(function(server) {
        ...
        // CHỐT CHẶN: XÓA SỔ SERVER T VIETSUB
        ...
        const items = Object.values(episodeMap).map(function(ep) {
            var cName = ep.name;
            // Fix đẹp tên tập phim nếu nó chỉ là số
...
// BẮT BUỘC: Khai báo biến này ở ngoài cùng của file script
...
function parseMovieDetail(html, url) {
    try {
        ...
        if (!isJsonCall) {
            ...
            // Lấy mô tả và lọc sạch mã Unicode (\u003cp\u003e)
            var rawDesc = _$(html).find(".prose.prose-sm").text();
            if (rawDesc) {
                rawDesc = rawDesc.replace(/\\u003c/gi, '<').replace(/\\u003e/gi, '>').replace(/\\u0026/gi, '&').replace(/\\u([\dA-Fa-f]{4})/g, (m, g) => String.fromCharCode(parseInt(g, 16)));
            ...
            // ==========================================================
            // TÌM CHÍNH XÁC ID CỦA PHIM (KHÔNG LẤY CỦA PHIM ĐỀ XUẤT)
            // ==========================================================
            var idVideo = null;
            var cleanHtml = html.replace(/\\"/g, '"').replace(/\\\\/g, '\\');
            
            // 1. Ưu tiên tìm trong object chính của bộ phim (chính xác 100%)
            var mainMovieMatch = cleanHtml.match(/"movie"\s*:\s*\{"id"\s*:\s*"?(\d+)"?/i);
            if (mainMovieMatch) {
                idVideo = mainMovieMatch[1];
            } else {
                // 2. Chốt ID bằng Slug trên thanh URL (cách ly với phim đề xuất)
                var currentSlug = url.split('/').filter(Boolean).pop().split('?')[0];
                var slugRegex = new RegExp('"id"\\s*:\\s*"?(\\d+)"?[^}]*?"slug"\\s*:\\s*"' + currentSlug + '"', 'i');
                var slugMatch = cleanHtml.match(slugRegex);
                
                if (!slugMatch) {
                    slugMatch = cleanHtml.match(new RegExp('"slug"\\s*:\\s*"' + currentSlug + '"[^}]*?"id"\\s*:\\s*"?(\\d+)"?', 'i'));
                }
            ...
            // =============================================================================
            // LƯỢT 2: NHẬN KẾT QUẢ JSON TỪ LINK EXTRA
            // =============================================================================
...
function parseDetailResponse(html, url) {
    try {
        ...
        // Truyền thẳng Link m3u8 vào cho Native Player
        return JSON.stringify({
            "url": streamUrl,
            "isEmbed": isEmbed,
            "headers": {
...
// =============================================================================
// BỘ PARSER MENU CỦA BẠN (GIỮ NGUYÊN)
// =============================================================================
