...
function getManifest() {
    return JSON.stringify({
        ...
        "description": "Bản Native iOS 17.0: Bắt Full 100% tập như Android, Xoá sạch T Vietsub.",
        "version": "17.0.0",
        "baseUrl": BASEURL,
        "iconUrl": BASEURL + "/motchill.png",
        "isEnabled": true,
        "type": "MOVIE",
        "playerType": "auto" // Dùng Native Player để triệt tiêu quảng cáo và auto-zoom trên iOS
    });
}
...
// =============================================================================
// THUẬT TOÁN LÕI CHI TIẾT: PORT TỪ ANDROID (2 LƯỢT QUÉT)
// =============================================================================
...
function transformMovieData(data) {
    ...
    data.servers.forEach(function(server) {
        ...
        // CHỐT CHẶN: XÓA SỔ HOÀN TOÀN SERVER T VIETSUB 
...
function parseMovieDetail(html, url) {
    try {
        ...
        if (!isJsonCall) {
            // =================================================================
            // LƯỢT 1: ĐỌC TRANG THÔNG TIN VÀ KHÓA CHÍNH XÁC ID PHIM (THEO KIỂU ANDROID)
            // =================================================================
            cachedMovieDetailId = id;
            var cleanHtml = html.replace(/\\"/g, '"').replace(/\\\\/g, '\\');

            // 1. TÌM CHÍNH XÁC MOVIE_ID ĐỂ GỌI API THEO PHƯƠNG PHÁP CỦA ANDROID
            var idVideo = null;
            var htmlMatch = html.match(/movie_id[^\d]+(\d+)/i) || cleanHtml.match(/movie_id[^\d]+(\d+)/i);
            if (htmlMatch) {
                idVideo = htmlMatch[1];
            } else {
                var altMatch = cleanHtml.match(/"movie"\s*:\s*\{"id"\s*:\s*"(\d+)"/i);
                if (altMatch) idVideo = altMatch[1];
            }
            ...
            // Xử lý mô tả: Quét thẻ Meta HTML thay vì JSON để tránh mã unicode \u003cp\u003e
            ...
            servers = transformMovieData(data); // Hàm ở trên sẽ tự xử vụ lọc T Vietsub

            extra = ""; // Đặt rỗng để báo App đã quét xong vòng lặp
        }
...
function parseDetailResponse(html, url) {
    try {
        ...
        // Truyền cho Player Native của iOS
        return JSON.stringify({
            "url": streamUrl,
            "isEmbed": isEmbed,
            "headers": {
