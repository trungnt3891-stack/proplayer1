...
function getManifest() {
    return JSON.stringify({
        ...
        "description": "Bản Duyệt Web VIP: Dành cho v1.8+, Lướt web gốc, Cố định dọc, Auto Login.",
        "version": "8.1.8", 
        "baseUrl": BASEURL,
        ...
        "layoutType": "VERTICAL",             
        "playerType": "webview"               // Chỉ mở bằng khung Webview
    });
}
...
function parseMovieDetail(html, url) {
    try {
        var lname = _$(html).find("h1").text() || _$(html).find('meta[property="og:title"]').attr("content") || "Đang cập nhật...";
        var limg = _$(html).find('meta[property="og:image"]').attr("content") || "";
        var ldes = _$(html).find(".order-6:content('Giới thiệu')").text() || "Không có mô tả.";
        ...
        var servers = [{
            name: "Duyệt Giao Diện Web",
            episodes: [{
                id: url, // TRUYỀN URL TRANG CHI TIẾT ĐỂ BÁC DUYỆT WEB MƯỢT MÀ
                name: "Mở Trình Duyệt Web",
                slug: "webview-player"
            }]
        }];
...
// =============================================================================
// WEBVIEW LOADER: ÉP CHẠY DỌC & AUTO LOGIN BẢO VỆ GIAO DIỆN WEB
// =============================================================================
function parseDetailResponse(html, url) {
    try {
        var preserveWebJs = `
            (function() {
                // 1. NGĂN CHẶN XOAY NGANG AN TOÀN CHO ANDROID WEBVIEW
                try {
                ...
                // 2. ẨN CÁC PHẦN TỬ THỪA, RÁC QUẢNG CÁO & NÚT FULLSCREEN
                var style = document.createElement('style');
                style.innerHTML = 'header, .header, nav, footer, .footer, .download-app, .app-download, [class*="ad-"], [id*="ad-"], .popup, .modal, .google-auto-placed, iframe[src*="ads"], .vjs-fullscreen-control, .plyr__controls [data-plyr="fullscreen"], .jw-fullscreen, video::-webkit-media-controls-fullscreen-button { display: none !important; opacity: 0 !important; pointer-events: none !important; z-index: -9999 !important; } body, html { overflow-x: hidden !important; }';
                document.head.appendChild(style);

                // 3. VÒNG LẶP LIÊN TỤC BƠM PLAYSINLINE ĐỂ KHÓA DỌC VIDEO
        ...
        return JSON.stringify({
            ...
            "headers": {
                ...
                "Custom-Js": preserveWebJs.replace(/\r\n|\r|\n/g, " ").trim()
            }
        });
    } catch (e) {
...
// =============================================================================
// [QUAN TRỌNG CHO V1.8] TRẢ VỀ URL RỖNG ĐỂ ÉP APP DỪNG LẠI Ở WEBVIEW[cite: 2]
// =============================================================================
function parseEmbedResponse(htmlContent, url) { 
    // Từ VAAPP 1.7.8 trở lên, url rỗng sẽ giữ lại Webview đang duyệt
