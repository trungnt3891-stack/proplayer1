...
function getManifest() {
    return JSON.stringify({
        ...
        "description": "Bản Webview v1.8: Khóa Xoay Ngang Native, Vuốt dọc tự chuyển tập, Chặn Quảng Cáo",
        "version": "1.8.0", 
        "baseUrl": BASEURL,
        ...
        "type": "shortfilm",                  // [KÍCH HOẠT CHẾ ĐỘ PHIM NGẮN TIKTOK][cite: 1]
        "layoutType": "VERTICAL",             // [ÉP AUTO XOAY DỌC MÀN HÌNH][cite: 1]
        "playerType": "webview"               // [CHUYỂN SANG WEBVIEW ĐỂ VUỐT][cite: 1]
    });
}
...
function parseMovieDetail(html, url) {
    try {
        ...
        ldirec = _$(html).find("span:content('Đạo diễn:')").parent().text().trim().replace("Diễn viên:", "");
...
// =============================================================================
// NÂNG CẤP XỬ LÝ RIÊNG CHO APP V1.8: ÉP CSS KHÓA ORIENTATION TRỰC TIẾP
// =============================================================================
function parseDetailResponse(html, url) {
    try {
        var fixVersion18Js = `
            (function() {
                // Ép cứng khung hình Webview luôn là chiều dọc (Portrait) chống xoay vật lý
                var forcePortraitStyle = document.createElement('style');
                forcePortraitStyle.innerHTML = 'header, .header, nav, footer, .footer, .download-app, .app-download, [class*="ad-"], [id*="ad-"], .popup, .modal, .google-auto-placed, iframe[src*="ads"], .bottom-nav, .navigation, .sidebar, .comments, .vjs-fullscreen-control, .plyr__controls [data-plyr="fullscreen"], .jw-fullscreen, .fullscreen-btn, video::-webkit-media-controls-fullscreen-button { display: none !important; opacity: 0 !important; pointer-events: none !important; z-index: -9999 !important; } body, html { margin: 0 !important; padding: 0 !important; width: 100% !important; height: 100% !important; overflow-x: hidden !important; background: #000 !important; orientation: portrait !important; }';
                document.head.appendChild(forcePortraitStyle);

                // Chặn triệt để sự kiện xoay màn hình ở mức DOM
                ...
                setInterval(function() {
                    // Ép tất cả các thẻ video nhận playsinline để tránh hệ thống tự động bung toàn màn hình ngang
        ...
        return JSON.stringify({
            ...
            "headers": {
                "Referer": BASEURL,
                "Custom-Js": fixVersion18Js.replace(/\r\n|\r|\n/g, " ").trim()
            }
        });
    } catch (e) {
