...
function getManifest() {
    return JSON.stringify({
        ...
        "description": "Bản Master: Fix lỗi Parse iOS, Kích hoạt Vuốt dọc tự động.",
        "version": "2.5.0", // Đổi version để ép App xóa cache
        "info": "Vuốt lên/xuống để qua tập và xem bằng chiều dọc.",
        "baseUrl": BASEURL,
        "iconUrl": "https://raw.githubusercontent.com/alokillgtv-gif/VAXAPPSCRIPT/main/img/shortflix.png",
        "isEnabled": true,
        "type": "shortfilm", // KÍCH HOẠT TỰ ĐỘNG XOAY DỌC VÀ VUỐT QUA TẬP
        "layoutType": "VERTICAL",
...
// CHỈ ĐỂ 5 MỤC NỔI BẬT Ở TRANG CHỦ ĐỂ TRÁNH LAG
...
function getUrlSearch(keyword, filtersJson) {
    return BASEAPI + "&q=" + encodeURIComponent(keyword.trim());
}
...
function parseListResponse(html, $url) {
    try {
        var items = [];
        // Xóa sạch các dấu hiệu lạ để parse được trên cả iOS và Android
        var cleanHtml = html.replace(/[\u0000-\u001F]+/g, ""); 
        var $data = JSON.parse(cleanHtml);
        var nextCursor = "";
...
// ĐÃ SỬA: Chuẩn hóa parseScript cho iOS (Loại bỏ các ký tự escape lồng nhau gây crash)
function parseScript(rawScript) {
    ...
    try {
        ...
        // Quét tìm cục JSON bắt đầu bằng "video":{
        ...
        if (videoIndex !== -1) {
            ...
            if (endIndex !== -1) {
                ...
                // Fix lỗi Unicode Escape trên iOS (ex: \u0022 -> ")
                const sanitizedStr = videoJsonStr.replace(/\\u([0-9a-fA-F]{4})/g, (match, grp) => String.fromCharCode(parseInt(grp, 16)));
                
                result.data = JSON.parse(sanitizedStr);
                result.success = true;
                return result;
            }
        }
    } catch (error) {
        // Fallback dùng Regex nếu parse thủ công lỗi
        try {
            let fallbackMatch = rawScript.match(/"video"\s*:\s*(\{.*?\})\s*,\s*"tags"/);
            if (fallbackMatch && fallbackMatch[1]) {
                let cl = fallbackMatch[1].replace(/\\"/g, '"').replace(/\\\\/g, '\\');
                result.data = JSON.parse(cl);
                result.success = true;
            }
        } catch (err2) {}
    }
...
function parseMovieDetail(html, url) {
    try {
        ...
        lname = _$(html).find("h1").text() || _$(html).find('meta[property="og:title"]').attr("content").split("|")[0].trim();
        limg = _$(html).find('meta[property="og:image"]').attr("content");
        ldes = _$(html).find('meta[property="og:description"]').attr("content") || _$(html).find(".order-6:content('Giới thiệu')").text();
        ...
        for (var $j = 0; $j < $listepi.length; $j++) {
            var $epicurrent = $listepi[$j];
            var $epinumber = $epicurrent.episodeNumber;
            
            // Xử lý thông minh tập 0 (Trailer)
            var $nameepi = $epinumber === 0 ? "Trailer / Tập 0" : "Tập " + $epinumber;
            
            // BẮT BUỘC ĐỂ AUTO VUỐT DỌC: ID phải trỏ THẲNG VÀO LINK .M3U8
            var $streamLink = "";
            if ($epicurrent.versions && $epicurrent.versions.length > 0) {
                $streamLink = $epicurrent.versions[0].videoUrl;
            }

            // Gắn link m3u8 vào id. Nếu không có m3u8, dùng tạm tham số url gốc để app tự dò
            var $itemId = $streamLink && $streamLink.startsWith("http") ? $streamLink : (url + "?tap=" + $epinumber);
...
// ĐÃ SỬA: Hàm cuối chỉ việc truyền lại cái m3u8 (id) đã bóc được ở trên cho Player phát
function parseDetailResponse(html, url) {
    try {
        var $stream = url;
        
        // Nếu URL truyền vào là link luồng phát native (.m3u8 hoặc .mp4)
        if ($stream.indexOf(".m3u8") !== -1 || $stream.indexOf(".mp4") !== -1) {
            return JSON.stringify({
                "url": $stream,
                "isEmbed": false, // KÍCH HOẠT VUỐT DỌC TIKTOK
        ...
        // Cú Fallback: Nếu không có m3u8, thử parse lại từ HTML (giống Code Android cũ)
...
// =============================================================================
// CATEGORIES & FILTER (GIỮ NGUYÊN DANH SÁCH MENU)
// =============================================================================
