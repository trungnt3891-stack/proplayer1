...
function getManifest() {
    return JSON.stringify({
        ...
        "playerType": "embed"                 // [ĐÃ SỬA] Đổi từ "webview" sang "embed" cho chuẩn v1.8
    });
}
...
// -----------------------------------------------------------------------------
// [ĐÃ SỬA] CHỌC MÙ TRÌNH DÒ LINK CỦA APP VÀ GIỮ GIAO DIỆN WEB
// -----------------------------------------------------------------------------
function parseDetailResponse(html, url) {
    try {
        var killAdsCssJs = `
            (function() {
                // [THÊM MỚI] Làm mù Sniffer để App không bắt được video
                var origQueryAll = document.querySelectorAll.bind(document);
                document.querySelectorAll = function(selector) {
                    if (typeof selector === 'string' && selector.toLowerCase().indexOf('video') !== -1) return [];
                    return origQueryAll(selector);
                };
                var origGetTags = document.getElementsByTagName.bind(document);
                document.getElementsByTagName = function(tag) {
                    if (typeof tag === 'string' && tag.toLowerCase() === 'video') return [];
                    return origGetTags(tag);
                };
                var origQuery = document.querySelector.bind(document);
                document.querySelector = function(selector) {
                    if (typeof selector === 'string' && selector.toLowerCase().indexOf('video') !== -1) return document.createElement('div');
                ...
                setInterval(function() {
                    // [THÊM MỚI] Ép video luôn phát chiều dọc để bù đắp việc tắt player app
                    var vids = origQueryAll('video');
                    ...
                    var appBanners = origQueryAll('div[class*="download"], div[class*="banner"]');
                    for (var i = 0; i < appBanners.length; i++) {
                        if (appBanners[i]) appBanners[i].style.display = 'none';
                    }
                    
                    // Tự động tắt nút Đóng popup nếu web tự mở ra quảng cáo
                    var closeBtns = origQueryAll('.close, .btn-close, [aria-label="Close"]');
        ...
        return JSON.stringify({
            ...
            "headers": {
                "Referer": BASEURL,
                // Loại bỏ khoảng trắng/xuống dòng an toàn cho chuẩn mới
                "Custom-Js": killAdsCssJs.replace(/\r\n|\r|\n/g, " ").trim()
            }
        });
    } catch (e) {
...
// Giữ nguyên chuẩn để ép WebView không trả lỗi 404
