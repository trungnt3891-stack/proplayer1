...
function getManifest() {
    return JSON.stringify({
        ...
        "description": "Bản Webview Gốc: Vuốt dọc tự chuyển tập, Tự động xoay dọc, Chặn Quảng Cáo",
        "version": "1.3.3", // Đã fix lỗi 404 & Chống xoay ngang an toàn
        "baseUrl": BASEURL,
...
function parseMovieDetail(html, url) {
    try {
        ...
        // Đẩy 1 item duy nhất để mở Webview gốc của bác
        servers.push({
...
// -----------------------------------------------------------------------------
// [BẢN GỐC FIX LỖI] Bổ sung tiêm playsinline và CSS ẩn nút Fullscreen
// -----------------------------------------------------------------------------
    ...
    try {
        var killAdsCssJs = `
            (function() {
                ...
                // [ĐÃ THÊM] Ẩn nút phóng to (Fullscreen) của trình phát Web để tránh bấm nhầm
                style.innerHTML = 'header, .header, nav, footer, .footer, .download-app, .app-download, [class*="ad-"], [id*="ad-"], .popup, .modal, .google-auto-placed, iframe[src*="ads"], .bottom-nav, .navigation, .sidebar, .comments, .vjs-fullscreen-control, .plyr__controls [data-plyr="fullscreen"], .jw-fullscreen, video::-webkit-media-controls-fullscreen-button { display: none !important; opacity: 0 !important; pointer-events: none !important; z-index: -9999 !important; } body, html { margin: 0 !important; padding: 0 !important; overflow-x: hidden !important; background: #000 !important; }';
                document.head.appendChild(style);

                setInterval(function() {
                    // [ĐÃ THÊM] Tự động tiêm playsinline vào video để Android không bốc video ra xoay ngang
        ...
        return JSON.stringify({
            ...
            "headers": {
                "Referer": BASEURL,
                // Chuyển mã JS thành 1 dòng an toàn để tránh lỗi Header HTTP
...
// =============================================================================
// [QUAN TRỌNG NHẤT] SỬA LỖI "KHÔNG TÌM THẤY TRANG (404)" Ở ĐÂY
// Trả lại đúng đường link url thay vì trả mảng rỗng (url: "")
// =============================================================================
