...
function getManifest() {
    return JSON.stringify({
        ...
        "description": "Kích hoạt Vuốt Dọc TikTok, Fix Lỗi Phân Tích JSON iOS",
        "version": "1.3.0", // Đổi version để ép App xóa cache
        ...
        "isEnabled": true,
        "type": "shortfilm",           // BẮT BUỘC ĐỂ KÍCH HOẠT VUỐT LÊN XUỐNG TIKTOK
        "layoutType": "VERTICAL",      // BẮT BUỘC ĐỂ XOAY DỌC MÀN HÌNH
...
// 1. CHỈ ĐỂ 5 MỤC HOT Ở TRANG CHỦ THEO YÊU CẦU
function getHomeSections() {
    return JSON.stringify([
        { "slug": "&sortBy=last_episode_at", "title": "Phim Mới Cập Nhật", "type": "Grid" },
        { "slug": "&genre=ngon-tinh", "title": "Ngôn Tình", "type": "Horizontal" },
        { "slug": "&genre=gia-dinh", "title": "Gia Đình", "type": "Horizontal" },
        { "slug": "&genre=chinh-kich", "title": "Chính Kịch", "type": "Horizontal" },
        { "slug": "&genre=hanh-dong", "title": "Hành Động", "type": "Horizontal" }
    ]);
}
...
// 2. FIX LỖI JSON.PARSE TRÊN IOS MÀ VẪN GIỮ NGUYÊN LOGIC CỦA BẠN
function parseScript(rawScript) {
    ...
    try {
        ...
        // Trích xuất an toàn mảng Next.js để tránh lỗi escape string trên iOS
        let matchNext = rawScript.match(/self\.__next_f\.push\((.*)\)/);
        if (matchNext) {
            try {
                let parsedArr = JSON.parse(matchNext[1]);
                cleaned = parsedArr[1]; // Lấy chuỗi raw chuẩn xác nhất
            } catch(e) {}
        } else {
            cleaned = rawScript.replace(/\\"/g, '"').replace(/\\\\/g, '\\');
        }
...
function parseMovieDetail(html, url) {
    try {
        ...
        quality = "HD";
        var yearStr = _$(html).find("span:content('Thời gian xuất bản:')").parent().text().trim().replace("Thời gian xuất bản:", "");
        ...
        for (var $j = 0; $j < $listepi.length; $j++) {
            ...
            // Xử lý thông minh Trailer (Tập 0)
...
// 3. CHỈNH SỬA MENU CÚ PHÁP ĐÚNG CỦA SHORTFLIX
function getLISTmenu() {
    return `
&sortBy=last_episode_at@@Phim Mới Cập Nhật
&genre=18-plus@@Phim 18+
&genre=action-&-adventure@@Action & Adventure
&genre=am-nhac@@Âm Nhạc
&genre=bi-an@@Bí Ẩn
&genre=chien-tranh@@Chiến Tranh
&genre=chinh-kich@@Chính Kịch
&genre=chuong-trinh-truyen-hinh@@Chương Trình Truyền Hình
&genre=dang-cap-nhat@@Đang Cập Nhật
&genre=gay-can@@Gây Cấn
&genre=gia-dinh@@Gia Đình
&genre=gia-tuong@@Giả Tưởng
&genre=hai-huoc@@Hài Hước
&genre=hanh-dong@@Hành Động
&genre=hoc-duong@@Học Đường
&genre=huyen-huyen@@Huyền Huyễn
&genre=khoa-hoc@@Khoa Học
&genre=kinh-di@@Kinh Dị
&genre=kinh-dien@@Kinh Điển
&genre=lang-man@@Lãng Mạn
&genre=lgbt@@LGBT
&genre=lich-su@@Lịch Sử
&genre=mien-tay@@Miền Tây
&genre=hai@@Phim Hài
&genre=ngan@@Phim Ngắn
&genre=nhac@@Phim Nhạc
&genre=short-drama@@Short Drama
&genre=sitcom@@Sitcom
&genre=tai-lieu@@Tài Liệu
&genre=talk@@Talk
&genre=tam-ly@@Tâm Lý
&genre=than-thoai@@Thần Thoại
&genre=than-tuong@@Thần Tượng
&genre=the-thao@@Thể Thao
&genre=tien-hiep@@Tiên Hiệp
&genre=tinh-cam@@Tình Cảm
&genre=tinh-tiet@@Tình Tiết
&genre=tinh-yeu-ngot-ngao@@Tình Yêu Ngọt Ngào
&genre=toi-pham@@Tội Phạm
&genre=tre-em@@Trẻ Em
