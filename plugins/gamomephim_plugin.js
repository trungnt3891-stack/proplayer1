...
// TỐI ƯU TUYỆT ĐỐI: MOI TRỰC TIẾP JSON NEXTJS + EXOPLAYER NATIVE (0% QUẢNG CÁO)
// =============================================================================
...
function getHomeSections() {
    var listurl = "[{\"link\":\"/phim-moi\",\"name\":\"Phim Mới\"}]";
...
function getUrlDetail(slug) {
    ...
    // Đảm bảo trỏ đúng vào đường dẫn chi tiết phim trên web
...
// BƯỚC QUAN TRỌNG: LẤY THÔNG TIN VÀ MOI TRỰC TIẾP DANH SÁCH M3U8/MP4 TỪ NEXTJS PAYLOAD
function parseMovieDetail(html, url) {
    try {
        ...
        // Nếu payload chưa quét được, thử quét trực tiếp qua Regex m3u8Url trong toàn bộ HTML
        var listepi = dataVD.episodes || [];
        if (listepi.length === 0) {
            var regexEp = /\{[^{}]*?"m3u8Url"\s*:\s*"([^"]+)"[^{}]*?\}/g;
            var matchEp;
            while ((matchEp = regexEp.exec(html)) !== null) {
                listepi.push({ m3u8Url: matchEp[1], audioType: "VIETSUB" });
            }
        }
        ...
        for (var $j = 0; $j < listepi.length; $j++) {
            ...
            if (link) {
                var epObj = {
                    "name": "Tập " + num,
                    "id": link, // Gán thẳng link video vào ID để phát Native
                    "slug": "tap-" + num
                };
                if (audio.indexOf("THUYET_MINH") > -1) {
                    itemsThuyetMinh.push(epObj);
                } else {
                    itemsVietsub.push(epObj);
                }
            }
        }
        
        if (itemsVietsub.length > 0) {
            servers.push({ "name": "Vietsub", "episodes": itemsVietsub });
        }
        if (itemsThuyetMinh.length > 0) {
            servers.push({ "name": "Thuyết Minh", "episodes": itemsThuyetMinh });
        }
        
        // Fallback phòng hờ nếu mảng rỗng
        if (servers.length === 0) {
            var fallbackMatch = html.match(/(https:\/\/[^"'\s]+\.mp4)/);
            if (fallbackMatch) {
                servers.push({
                    "name": "Phát Ngay",
                    "episodes": [{ "name": "Tập 1", "id": fallbackMatch[1], "slug": "tap-1" }]
                });
            }
        }
...
// EXOPLAYER NATIVE SẼ PHÁT TRỰC TIẾP LINK LẤY ĐƯỢC TỪ ID
function parseDetailResponse(html, url) {
    try {
        var mimeType = url.indexOf(".m3u8") > -1 ? "application/x-mpegURL" : "video/mp4";
        return JSON.stringify({
            "url": url,
            "isEmbed": false, // False để ép ExoPlayer phát trực tiếp dạng dọc (Shortfilm)
            "mimeType": mimeType,
            "headers": {
                "Referer": BASEURL,
                "Origin": BASEURL,
