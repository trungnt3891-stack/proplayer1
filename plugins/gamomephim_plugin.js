...
function getManifest() {
    return JSON.stringify({
        "id": "gamomephim",
        "name": "Gà Mờ Mê Phim",
        "description": "Fix Tìm Kiếm & Cập nhật đầy đủ danh mục Thể Loại.",
        "version": "1.2.0",
        "baseUrl": BASEURL, // Đổi "BASEURL" (viết hoa) thành "baseUrl" chuẩn VAX
...
// CHIA LẠI FOLDER TRANG CHỦ THEO ẢNH
...
function getUrlList(slug, filtersJson) {
    try {
        ...
        if (path) {
            // Đảm bảo không bị trùng dấu // khi nối chuỗi
...
// ĐÃ SỬA: Cập nhật cấu trúc tìm kiếm mới nhất của Gà Mờ Mê Phim (?q=)
function getUrlSearch(keyword, filtersJson) {
    return BASEURL + "/tim-kiem?q=" + encodeURIComponent(keyword.trim());
}
...
function parseListResponse(html, $url) {
    try {
        ...
        // ĐÃ SỬA: Bỏ class .relative do cấu trúc web đã gỡ class này
        _$(html).find(".grid").find("a").each(function() {
            var href = this.attr("href");
            
            // NGĂN LỖI: Lọc bỏ các thẻ <a> là menu/header, chỉ lấy link phim
            ...
            // Cố gắng bắt title an toàn
            var title = this.attr("title");
            if(!title) title = this.find("h3").text() || "";
            
            // Cố gắng bắt ảnh an toàn
            var src = this.find("img").attr("src");
            if(!src) src = this.find("img").attr("data-src") || "";
...
// Bắt kết quả tìm kiếm dùng chung hàm parseListResponse đã được sửa
...
// CẬP NHẬT: Danh bạ 15 Thể loại mới nhất dựa trên ảnh của Web
